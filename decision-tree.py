...
def buildtree(rows, scoref = entropy):
    ...
    # 经验熵
    empiricial_entropy = entropy(rows)
    ...
    for col in range(column_count):
        ...
        column_best_gain = 0.0
        column_best_criteria = None
        column_best_sets = None
        ...
        for value in column_values:
            ...
            condition_entropy += p * entropy(set1)
            column_entroy += -p * log(p, 2)

            # 某一个值划分信息增益
            gain = empiricial_entropy - p*scoref(set1) - (1-p)*scoref(set2)

            # 根据信息增益值选择最合适的值
            if gain > column_best_gain and len(set1)>0 and len(set2)>0:
                column_best_gain = gain
                column_best_criteria = (col, value)
                column_best_sets = (set1, set2)

        if column_entroy == 0:
            continue

        # 选择信息增益比例较大的特征列
        gain = empiricial_entropy - condition_entropy   
        gain_ratio = gain / column_entroy
        print gain, column_entroy, gain_ratio

        if gain_ratio > best_gain and len(column_best_sets[0])>0 and len(column_best_sets[1])>0:
            print 'The best gain ration is:', gain_ratio, 'by:', column_best_criteria, '\n->tb:', column_best_sets[0],'\n->fb:', column_best_sets[1]
            best_gain = gain_ratio
            best_criteria = column_best_criteria
            best_sets = column_best_sets
